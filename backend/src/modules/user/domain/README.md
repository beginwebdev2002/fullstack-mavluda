...
*Breadcrumbs:* [backend](/backend) / [src](/backend/src) / [modules](/backend/src/modules) / [user](/backend/src/modules/user) / [domain](/backend/src/modules/user/domain)
...
```mermaid
graph TD
  Root --> F0(📄 user.entity.ts)
```

## 📄 FILE REGISTRY
| File Name | Type | Responsibility | Key Aliases Used |
|---|---|---|---|
| `user.entity.ts` | `ts` | Core functionality | `None` |
...
import { example } from './user.entity.ts';
```
