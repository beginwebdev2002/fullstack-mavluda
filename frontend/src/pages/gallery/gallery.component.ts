...
import { Gallery, ImageCategory } from '../../entities/gallery/gallery.model';
...
export class GalleryComponent implements OnInit {
  ...
  saveImage() {
    if (!this.currentImage.id) { // New image
      // Backend handles createdAt
      this.galleryService.createImage(this.currentImage).subscribe(() => {
         this.closeModal();
      });
    } else { // Update existing
      this.galleryService.updateImage(this.currentImage.id, this.currentImage).subscribe(() => {
         this.closeModal();
      });
    }
  }
