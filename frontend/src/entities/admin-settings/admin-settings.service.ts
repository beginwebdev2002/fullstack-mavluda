...
export class AdminSettingsService {
  ...
  createSettings(
    ...
    return this.http
      .post<AdminSettings>(this.apiUrl, settings)
      .pipe(tap((newSettings) => this._settings.set(newSettings)));
  }

  updateSettings(settings: Partial<AdminSettings>): Observable<AdminSettings> {
    return this.http
      .put<AdminSettings>(this.apiUrl, settings)
      .pipe(tap((updatedSettings) => this._settings.set(updatedSettings)));
  }

  deleteSettings(): Observable<boolean> {
    return this.http
      .delete<boolean>(this.apiUrl)
      .pipe(tap(() => this._settings.set(null)));
  }
}
