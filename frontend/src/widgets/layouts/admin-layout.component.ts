    ...
    <div class="flex h-screen overflow-hidden bg-white dark:bg-background-dark">
      ...
      @if (isMobileSidebarOpen()) {
        ...
        >
          <!-- Backdrop -->
          <div
            class="fixed inset-0 bg-black/60 backdrop-blur-sm transition-opacity animate-fade-in"
            (click)="toggleMobileSidebar()"
          ></div>

          <!-- Sidebar Panel -->
          <div
            class="relative flex flex-col w-72 max-w-xs h-full bg-[#050505] shadow-2xl transform transition-transform animate-fade-in"
          >
            <!-- Sidebar Content -->
          ...
          <div class="p-4 animate-fade-in">
            <button
              (click)="toggleMobileSidebar()"
              class="text-white bg-white/10 hover:bg-white/20 p-2 rounded-full transition-colors backdrop-blur-md"
            >
